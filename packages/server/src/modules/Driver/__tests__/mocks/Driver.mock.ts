...
export const DriverMocks: Omit<DriverType, 'constructor_entity'>[] = new Array(2)
    .fill(null)
    .map((_, index) => ({
        id: index + 1,
        ref: `driver_${index + 1}`,
...
export const DriverResultsMock: Omit<ResultType, 'driver' | 'constructor_entity' | 'status'>[] =
    new Array(2).fill(null).map((_, index) => ({
        ...
        race_id: 1,
        race: null,
        driver_id: DriverMocks[index].id,
        constructor_id: 1,
        driver_number: 1,
...
export const DriverQualifyingMock: QualifyingType[] = new Array(2).fill(null).map((_, index) => ({
    id: index + 1,
    race_id: 1,
    driver_id: DriverMocks[index].id,
    constructor_id: 1,
